...
2. Supabase **Connect → Session pooler** bağlantısını açın. Host, kullanıcı ve portu buradan kopyalayın. **Database Settings → SSL** bölümünden kök sertifikayı indirin. Render için `ConnectionStrings__DefaultConnection` değerini Npgsql biçiminde girin: `Host=POOLER_HOST;Port=5432;Database=postgres;Username=postgres.PROJECT_REF;Password=DB_PASSWORD;SSL Mode=VerifyFull;Maximum Pool Size=10`. Direct/IPv6 veya Transaction pooler/6543 bağlantısını kullanmayın.

...
const TEMPLATE_COLUMNS: { name: string; hint: string; required?: boolean }[] = [
  ...
  { name: 'Soyad', hint: 'Zorunlu', required: true },
  { name: 'SicilNo', hint: 'Benzersiz personel numarası' },
  { name: 'Cinsiyet', hint: 'Erkek / Kadın / Diğer' },
  { name: 'Durum', hint: 'Aktif, Pasif, İzinli…' },
  { name: 'DogumTarihi', hint: 'YYYY-AA-GG' },
  { name: 'Telefon', hint: '05XXXXXXXXX' },
  { name: 'Eposta', hint: 'ornek@kurum.local' },
  { name: 'BirimKodu', hint: 'Referans sayfasından', required: true },
  { name: 'UnvanAdi', hint: 'Referans sayfasından' },
  { name: 'IseGirisTarihi', hint: 'YYYY-AA-GG' },
  { name: 'TCKN', hint: 'Yetkiliyse doldurulur' },
]
...
export function EmployeeImportPage() {
  ...
  function pickFile(f: File | null) {
    ...
    if (!isXlsx) {
      setError('Yalnızca .xlsx uzantılı dosya yükleyebilirsiniz.')
      return
    }
    setError(null)
    setResult(null)
  ...
  async function onSubmit(e: FormEvent) {
    e.preventDefault()
    if (!file) {
      setError('Bir .xlsx dosyası seçin.')
      return
    }
    if (!canCreate) {
      setError('Aktarım için personel ekleme yetkisi de gerekir.')
      return
    }
    setBusy(true)
    setError(null)
    ...
    <div className="org-page import-page">
      <header className="org-hero panel">
        <div>
          ...
          <h1>Toplu personel yükleme</h1>
          <p className="muted">
            Mevcut personel listelerini Excel ile hızlıca sisteme aktarın. Her satır ayrı
            doğrulanır; hatalı satırlar atlanır, geçerli satırlar kaydedilir.
          </p>
        </div>
          ...
          <li>
            ...
            <div>
              ...
              <p className="muted small">
                Personeller, Referans (birim/unvan) ve Yardım sayfalarını içerir.
              </p>
            </div>
          </li>
          <li>
            ...
            <div>
              ...
              <p className="muted small">
                Örnek satırı düzenleyin; BirimKodu ve UnvanAdi için Referans sayfasına bakın.
              </p>
            </div>
          </li>
          <li>
            ...
            <div>
              ...
              <p className="muted small">
                Sonuç raporunda her satırın durumunu ve hatalarını görürsünüz.
              </p>
            </div>
          </li>
        </ol>
      ...
      {!canCreate && (
        <div className="panel import-notice">
          <strong>Yalnızca şablon indirebilirsiniz</strong>
          <p className="muted small">
            Aktarımı başlatmak için personel ekleme yetkisi gerekir. Şablonu indirip
            hazırlayabilirsiniz.
          </p>
        </div>
      )}
      ...
      <section className="panel import-upload-card">
        <form onSubmit={(e) => void onSubmit(e)}>
          ...
          >
            ...
            {file ? (
              ...
              <>
                ...
                <p className="muted small">Yalnızca .xlsx · en fazla 500 satır</p>
              </>
            )}
          </div>
          ...
          <div className="import-upload-actions">
            <button type="submit" className="btn-primary" disabled={busy || !canCreate || !file}>
      ...
      <section className="panel import-columns-card">
        <div className="dq-section-head">
          ...
          <p className="muted small">Beklenen alanlar ve biçimler</p>
        </div>
          ...
          {TEMPLATE_COLUMNS.map((c) => (
            <div key={c.name} className="import-column">
              <span className="import-column-name">
                {c.name}
                {c.required && <span className="import-column-req">•</span>}
              </span>
            ...
            <div>
              ...
              <p className="muted small">
                {result.failureCount === 0
                  ...
                  : 'Hatalı satırları düzeltip yalnızca o satırları tekrar yükleyebilirsiniz.'}
              </p>
            </div>
                  ...
                  <tr key={r.rowNumber} className={r.success ? undefined : 'is-error'}>
                    ...
                    <td>
                      ...
                      {r.employeeId && (
                        <>
                          {' '}
                          <Link to={`/employees/${r.employeeId}`} className="import-row-link">
                            profili aç
                          </Link>
                        </>
                      )}
